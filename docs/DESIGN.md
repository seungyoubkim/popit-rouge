...
버블을 팝할 때 다음 피드백이 동시에 발생한다. 콤보 카운트(최대 10단계)에 따라 효과가 점진적으로 강해진다.

#### 피드백 요소
| 요소 | 설명 |
|------|------|
| **스케일 펀치** | 팝 순간 버블이 살짝 커졌다가 줄어드는 애니메이션 |
| **화면 흔들림** | 화면 전체가 짧게 흔들림 (콤보에 따라 강도/지속 증가) |
| **파티클** | 버블 색상의 작은 입자가 사방으로 튀어나감 (콤보에 따라 수/속도/크기 증가) |
| **플래시** | 팝 위치에 하얀 원이 번쩍임 (콤보에 따라 크기/지속 증가) |
| **진동** | 50ms 고정 햅틱 피드백 |
...
| 플래시 지속 | 120ms | 170ms |
| 화면 흔들림 강도 | 3px | 8px |
| 화면 흔들림 지속 | 80ms | 130ms |

#### 라운드 전환 규칙
- 라운드 종료(승리/패배) 시점에 이미 발생한 파티클, 플래시, 화면 흔들림 애니메이션은 끝까지 재생된 후 정리된다.
